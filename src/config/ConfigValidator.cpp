...
const std::set<std::string> ConfigValidator::VALID_COLORS = {"COKLAT",
                                                             "BIRU_MUDA",
                                                             "MERAH_MUDA",
                                                             "ORANGE",
                                                             "MERAH",
...
void ConfigValidator::validateProperties(
    const Config& config, std::vector<ConfigValidationError>& errors) {
...
void ConfigValidator::validatePropertyTypes(
    ...
    for (const auto& prop : properties) {
        if (VALID_PROPERTY_TYPES.find(prop.type) ==
            VALID_PROPERTY_TYPES.end()) {
            addError(errors,
                     "property.txt",
                     prop.id,
                     "TYPE",
    ...
    for (const auto& tile : actionTiles) {
        ...
        if (tile.id < MIN_BOARD_POSITION || tile.id > MAX_BOARD_POSITION) {
            addError(errors,
                     ...
                     "ID must be between " +
                         std::to_string(MIN_BOARD_POSITION) + " and " +
                         std::to_string(MAX_BOARD_POSITION));
        }
    ...
    for (int i = 1; i <= EXPECTED_RAILROAD_COUNT; ++i) {
        if (railroad.rentTable.find(i) == railroad.rentTable.end()) {
            addError(errors,
                     ...
                     "Must define rent for " + std::to_string(i) +
                         " railroad(s)");
        }
    }
...
void ConfigValidator::validateTax(const Config& config,
                                  std::vector<ConfigValidationError>& errors) {
...
void ConfigValidator::validateMisc(const Config& config,
                                   std::vector<ConfigValidationError>& errors) {
...
void ConfigValidator::addError(std::vector<ConfigValidationError>& errors,
