...
Player::Player(std::string username, int balance)
    : username(username), status(PlayerStatus::ACTIVE), balance(balance),
      position(0), jailTurns(0), usedSkillThisTurn(false), discountRate(0),
      properties({}), hand({}) {}
...
bool Player::isInJail() const {
    return this->getStatus() == PlayerStatus::JAILED;
}

int Player::getDiscountRate() const {
    return this->discountRate;
}

int Player::getPropertiesAmount() const {
    return this->properties.size();
}

int Player::getHandsAmount() const {
...
int Player::move(int steps, TurnContext& ctx) {
    // if (ctx.board.getSize() == nullptr) { throw
    // InvalidGameStateException("Board size source is not set for player
    // movement.");}
    if (ctx.board.getSize() <= 0) {
        throw InvalidGameStateException("Board size must be positive.");
    }
    const int trueSteps =
        ((steps % ctx.board.getSize()) + ctx.board.getSize()) %
        ctx.board.getSize();
...
void Player::drawSCard(SkillCard* card) {
    ...
    std::cout << "[MENDAPATKAN KEMAMPUAN] " << card->getDescription()
...
void Player::activateShield() {
    if (this->isShieldActive) {
        throw InvalidGameStateException("Shield is already active for player " +
                                        this->username);
    }
