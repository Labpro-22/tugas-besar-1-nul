...
int BotPlayer::decideBid(int currentBid,
                         int propertyValue,
                         int currentBalance) {
...
bool BotPlayer::decideMortgage(int mortgageValue) {
    ...
    displayBotAction("Menggadaikan properti untuk mendapatkan M" +
...
int BotPlayer::decideJailAction(int jailFine,
    ...
    case 3: // Hard - Pay to get out quickly
        if (currentBalance >= jailFine + 200) {
            displayBotAction("Membayar denda M" + std::to_string(jailFine) +
                             " untuk bebas cepat!");
            return 1;
        }
        break;
