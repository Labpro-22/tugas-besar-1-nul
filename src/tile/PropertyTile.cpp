...
void StreetTile::triggerBuyOrAuction(TurnContext& ctx) {
    ...
    while (true) {
        ...
        if (ans == "Y" || ans == "y") {
            ...
            cout << "[" << player.getUsername() << "] baru saja membeli "
                 << getName() << "\n";
            cout << "Uang [" << player.getUsername() << "] tersisa: "
                 << player.getBalance() - getProperty()->getBuyPrice()
...
void StreetTile::triggerRentPayment(TurnContext& ctx) {
    ...
    cout << "[" << player.getUsername() << "] mendarat di ["
         << getProperty()->getName() << "] milik ["
         << getProperty()->getOwner()->getUsername() << "].\n\n";
    getProperty()->printStatus(ctx);
    cout << "Uang anda tersisa: <M"
         << player.getBalance() - getProperty()->getRent(ctx)
         << ">.\n\n"; // nanti implement dari player, biar bisa kurangi balance
                      // player
}
