...
void Board::buildFromConfig(const Config& config) {
    ...
    auto addStreet = [this](int idx,
        ...
        auto prop = make_unique<StreetProperty>(code,
                                                name,
                                                buyPrice,
                                                mortgageValue,
                                                color,
    ...
    auto addUtility = [this, &config](int idx,
                                      ...
                                      int mortgageValue) {
        auto prop =
            make_unique<UtilityProperty>(code,
                                         name,
                                         buyPrice,
                                         mortgageValue,
                                         config.utility.multiplierTable);
    ...
    for (const ActionTileData& action : config.actionTiles) {
        ...
        if (action.isFestival()) {
            setTileAt(idx,
                      make_unique<FestivalTile>(idx, action.code, actionName));
        } else if (action.isTax()) {
            const TaxType taxType =
                (action.code == "PPH") ? TaxType::PPH : TaxType::PBM;
            setTileAt(
                idx,
                make_unique<TaxTile>(idx, action.code, actionName, taxType));
        } else if (action.isChanceCard() || action.isCommunityChest()) {
            const bool isChance = action.isChanceCard();
            setTileAt(
                idx,
                make_unique<CardTile>(idx, action.code, actionName, isChance));
        } else if (action.isSpecial()) {
            if (action.code == "GO") {
                setTileAt(idx,
                          make_unique<GoTile>(idx, action.code, actionName));
            } else if (action.code == "PEN") {
                setTileAt(idx,
                          make_unique<JailTile>(idx, action.code, actionName));
            } else if (action.code == "BBP") {
                setTileAt(
                    idx,
                    make_unique<FreeParkingTile>(idx, action.code, actionName));
            } else if (action.code == "PPJ") {
                setTileAt(
                    idx,
                    make_unique<GoToJailTile>(idx, action.code, actionName));
            } else {
                setTileAt(
                    idx, make_unique<ActionTile>(idx, action.code, actionName));
            }
        }
    }
}
...
void Board::generateDefaultBoard() {
    ...
    auto addRailroad = [this](int idx,
        ...
        auto prop = make_unique<RailroadProperty>(
            code, name, buyPrice, mortgageValue, rentTable);
        RailroadProperty& ref = *prop;
        properties.push_back(std::move(prop));
        setTileAt(idx, make_unique<RailroadTile>(idx, ref));
    };
    ...
    auto addUtility = [this](int idx,
        ...
        auto prop = make_unique<UtilityProperty>(
            code, name, buyPrice, mortgageValue, multTable);
        UtilityProperty& ref = *prop;
        properties.push_back(std::move(prop));
        setTileAt(idx, make_unique<UtilityTile>(idx, ref));
    };
    ...
    addStreet(
        1, "GRT", "Garut", 60, 30, "Coklat", 50, 50, {2, 10, 30, 90, 160, 250});
    ...
    addStreet(3,
              ...
              {4, 20, 60, 180, 320, 450});
    setTileAt(
        4, make_unique<TaxTile>(4, "PPH", "Pajak Penghasilan", TaxType::PPH));
    addRailroad(5,
                "GBR",
                "Stasiun Gambir",
                200,
    ...
    setTileAt(
        38,
        make_unique<TaxTile>(38, "PBM", "Pajak Barang Mewah", TaxType::PBM));
    addStreet(39,
              "IKN",
              "Ibu Kota Nusantara",
              400,
