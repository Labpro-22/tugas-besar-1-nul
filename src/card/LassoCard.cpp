...
void LassoCard::apply(TurnContext& ctx) {
    ...
    std::cout
        << "[MENGGUNAKAN] LassoCard: Menarik pemain lawan ke posisi ini.\n";

    std::cout << "\n=== Daftar Pemain yang Berada di Depan ===\n";
    std::cout << "Pemain saat ini: " << player.getUsername()
              << " (posisi: " << currentPos << ")\n";
    ...
    while (!(std::cin >> choice) || choice < 1 ||
        ...
        std::cout
            << "[INFO] Pilihan tidak valid. Masukkan nomor target yang benar: ";
    }
