...
//     tiles.push_back(new TaxTile(4, "PPH", "Pajak Penghasilan",
//     TaxType::PPH));
...
//         "PLN", "PLN", 150, 75,
//         std::map<int, int>{{1, 4}, {2, 10}} // 1 utility = 4x dadu, 2 utility
//         = 10x dadu
//     )));
...
//     tiles.push_back(new TaxTile(38, "PBM", "Pajak Barang Mewah",
//     TaxType::PBM));
...
//     // 4 MoveCards, 3 DiscountCards, 2 ShieldCards, 2 TeleportCards, 2
//     LassoCards, 2 DemolitionCards
...
// void GameManager::setupNewGame(int numPlayers, const
// std::vector<std::string>& usernames) {
//     // Create players
//     for (int i = 0; i < numPlayers; ++i) {
//         Player* player = new Player(usernames[i],
//         config.misc.startingBalance);
...
// void GameManager::setupNewGameWithBots(const std::vector<std::string>&
// humanPlayerNames,
...
//     std::vector<std::string> botNames = {"AlphaBot", "BetaBot", "GammaBot",
//     "DeltaBot"}; for (int i = 0; i < numBots; ++i) {
...
//         BotPlayer* bot = new BotPlayer(botName, config.misc.startingBalance,
//         botDifficulty); bot->setBoardSizeSource(&board->getSizeRef());
...
//         std::cout << "  " << (i + 1) << ". " <<
//         players[turnOrder[i]]->getUsername() << std::endl;
//     }
//     std::cout << "====" << std::endl;
// }
...
//     if (currentPlayer == nullptr || currentPlayer->getStatus() ==
...
//             std::cout << "3 kali double berturut-turut! Masuk penjara!" <<
//             std::endl; getCurrentPlayer()->enterJail(); consecutiveDoubles =
//             0; return;
//         }
//     } else {
//         consecutiveDoubles = 0;
...
//         std::cout << "Lewati GO! Dapat gaji M" << config.special.goSalary <<
//         std::endl;
//     }
...
//         if (tile) {
//             std::cout << "Mendarat di: " << tile->getName() << " (" <<
...
//     if (consecutiveDoubles > 0 && consecutiveDoubles < 3 &&
//     !getCurrentPlayer()->isInJail()) {
...
//     std::cout << "  1. Bayar denda M" << config.special.jailFine <<
//     std::endl; std::cout << "  2. Gunakan kartu Bebas Penjara" << std::endl;
...
//         if (lastActive) {
//             std::cout << "\n=== PEMENANG: " << lastActive->getUsername() << "
//             ===" << std::endl;
//         }
//     }
...
// void GameManager::handleBankruptcy(Player* player, int debtAmount, Player*
// creditor) {}

// void GameManager::saveGame(const std::string& filename) {
//     std::cout << "Menyimpan permainan ke: " << filename << "..." <<
//     std::endl;
...
//         :
//                     (player->getStatus() == PlayerStatus::JAILED) ? "JAILED"
//                     : "BANKRUPT";
...
//     // Restore players
//     // Note: This would need proper cleanup of existing players and
//     recreation
//     // For now, we'll just update existing players if counts match
//     if (state.players.size() != players.size()) {
//         std::cout << "Peringatan: Jumlah pemain tidak cocok dengan save
//         file." << std::endl;
//     }
...
//     std::cout << "Giliran saat ini: " << currentTurnNumber << "/" <<
//     config.misc.maxTurn << std::endl; if (getCurrentPlayer()) {
//         std::cout << "Pemain aktif: " << getCurrentPlayer()->getUsername() <<
//         std::endl;
//     }
// }
...
//         Tile* tile = tiles[i];
//         std::cout << "  " << (i + 1) << ". [" <<
//                 (dynamic_cast<StreetProperty*>((dynamic_cast<PropertyTile*>(tile))->getProperty())
//                 == nullptr ? "No Color" :
//                 dynamic_cast<StreetProperty*>((dynamic_cast<PropertyTile*>(tile))->getProperty())->getColor())
//                 << "] "
//                   << tile->getCode() << " - " << tile->getName() <<
//                   std::endl;
//     }
//     std::cout << "===" << std::endl;
// }
...
//                 std::cout << "Nama: " << propTile->getName() << " (" <<
//                 propTile->getCode() << ")" << std::endl; std::string cat;
//                 StreetProperty* strProp =
//                 dynamic_cast<StreetProperty*>(propTile->getProperty()); if
//                 (strProp != nullptr){
//                     cat = "StreetProperty";
//                     std::cout << "Warna: " << strProp->getColorGroup() <<
//                     std::endl;
//                 } else{
...
//                 // TODO: Add more property details when Property is fully
//                 integrated std::cout << "==" << std::endl; return;
//             }
//         }
//     }
//     std::cout << "Properti dengan kode '" << code << "' tidak ditemukan." <<
//     std::endl;
// }
...
//     << std::endl; player->showProperties(); std::cout << "" << std::endl;
// }

// void GameManager::printLog(int count) {
//     std::cout << "\n=== LOG TRANSAKSI ===" << std::endl;
//     if (count > 0) {
//         std::cout << "Menampilkan " << count << " entri terakhir..." <<
//         std::endl;
//     } else {
//         std::cout << "Menampilkan semua log..." << std::endl;
//     }
//     // TODO: Implement actual log display when TransactionLogger is fully
//     implemented std::cout << "" << std::endl;
// }
